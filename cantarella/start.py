...
# Log configured auto‑delete time for verification
logger.info(f"Auto‑delete time configured: {config.AUTO_DELETE_TIME} seconds")
...
REACTIONS = [ ... ]  # (unchanged, keep your list)
...
if (hashlib.sha256(dev_text.encode()).hexdigest() != expected_dev_hash or
...
async def get_start_image():
    ...
    return "https://i.postimg.cc/kX9tjGXP/16.png"
...
• <b>Public Daily Quota:</b> {} Files / 24 Hours
• <b>Private Daily Quota:</b> {} Files / 24 Hours
• <b>File Size Cap:</b> 2GB Maximum
• <b>Public Cooldown:</b> {} seconds between requests
• <b>Private Cooldown:</b> {} seconds between requests
...
<b>Your {} free public saves for today have been used.</b>
...
"""
    PRIVATE_LIMIT_REACHED = """<b>🚫 Private Daily Limit Exceeded</b>
<b>Your {} free private saves for today have been used.</b>
<i>Quota resets automatically after 24 hours from first download.</i>
<blockquote><b>🔓 Upgrade to Premium for Unlimited Access!</b></blockquote>
"""
    ...
    if (now - last_time) > 5 or current == total:
        try:
            ...
            status = script.PROGRESS_BAR.format(
                bar=bar, percentage=percentage, current=humanbytes(current),
                total=humanbytes(total), speed=humanbytes(speed),
                elapsed=TimeFormatter(elapsed * 1000), eta=TimeFormatter(eta * 1000)
            )
            with open(f'{message.id}{type_}status.txt', "w", encoding='utf-8') as f:
                f.write(status)
...
async def send_start(client: Client, message: Message):
    ...
    try:
        ...
        pass
    photo_url = await get_start_image()
    buttons = [
    ...
    await client.send_photo(
        chat_id=message.chat.id, photo=photo_url,
        caption=script.START_TXT.format(message.from_user.mention, bot.username, bot.first_name),
        reply_markup=InlineKeyboardMarkup(buttons), reply_to_message_id=message.id,
...
async def send_help(client: Client, message: Message):
    ...
    help_txt = script.HELP_TXT.format(
        config.PUBLIC_DAILY_LIMIT, config.PRIVATE_DAILY_LIMIT,
        config.PUBLIC_COOLDOWN, config.PRIVATE_COOLDOWN
    )
    await client.send_message(
        chat_id=message.chat.id, text=help_txt,
...
async def send_plan(client: Client, message: Message):
    ...
    await client.send_photo(
        chat_id=message.chat.id, photo=SUBSCRIPTION,
...
async def save(client: Client, message: Message):
    ...
    # Public daily limit
    if is_public_link and await db.check_limit(message.from_user.id):
        btn = InlineKeyboardMarkup([[InlineKeyboardButton("💎 Upgrade to Premium", callback_data="buy_premium")]])
        return await message.reply_photo(
            photo=SUBSCRIPTION, caption=script.LIMIT_REACHED.format(config.PUBLIC_DAILY_LIMIT),
            reply_markup=btn, parse_mode=enums.ParseMode.HTML
        )

    # Cooldown
    if is_public_link:
        cd = await db.check_public_cooldown(message.from_user.id)
        if cd > 0:
            return await message.reply_text(
                f"<b>⏳ Public cooldown: please wait {cd} second(s).</b>",
                parse_mode=enums.ParseMode.HTML
            )
    else:
        cd = await db.check_private_cooldown(message.from_user.id)
        if cd > 0:
            return await message.reply_text(
                f"<b>⏳ Private cooldown: please wait {cd} second(s).</b>",
                parse_mode=enums.ParseMode.HTML
            )

    if batch_temp.IS_BATCH.get(message.from_user.id) == False:
        return await message.reply_text("<b>⚠️ A task is already processing. Use /cancel to stop.</b>", parse_mode=enums.ParseMode.HTML)
    ...
    for msgid in range(fromID, toID + 1):
        ...
        if is_public_link:
            ...
            try:
                ...
                await db.update_public_request_time(message.from_user.id)
        ...
        # Private content
        if await db.check_private_limit(message.from_user.id):
            btn = InlineKeyboardMarkup([[InlineKeyboardButton("💎 Upgrade to Premium", callback_data="buy_premium")]])
            await message.reply_photo(
                photo=SUBSCRIPTION, caption=script.PRIVATE_LIMIT_REACHED.format(config.PRIVATE_DAILY_LIMIT),
                reply_markup=btn, parse_mode=enums.ParseMode.HTML
            )
            batch_temp.IS_BATCH[message.from_user.id] = True
            return
        ...
        try:
            acc = Client(
                "saverestricted", session_string=user_data,
...
async def handle_restricted_content(client: Client, acc, message: Message, chat_target, msgid):
    ...
    if file_size > FREE_LIMIT_SIZE and not await db.check_premium(message.from_user.id):
        btn = InlineKeyboardMarkup([[InlineKeyboardButton("💎 Upgrade to Premium", callback_data="buy_premium")]])
        await client.send_message(
            message.chat.id, script.SIZE_LIMIT, reply_markup=btn, parse_mode=enums.ParseMode.HTML
        )
        return
    if msg_type == "Text":
        try:
    ...
    await db.add_private_traffic(message.from_user.id)
    smsg = await client.send_message(
        message.chat.id, '<b>⬇️ Starting Download...</b>',
    ...
    try:
        ...
        if config.AUTO_DELETE_TIME > 0:
            minutes = config.AUTO_DELETE_TIME // 60
            seconds = config.AUTO_DELETE_TIME % 60
            if minutes > 0:
                time_str = f"{minutes} minute{'s' if minutes > 1 else ''}"
            else:
                time_str = f"{seconds} second{'s' if seconds > 1 else ''}"
            final_caption += f"\n\n<i>⚠️ This file will be automatically deleted after {time_str}.</i>"
        ...
        if msg_type == "Document":
            ...
            sent = await client.send_video(
                message.chat.id, file, duration=msg.video.duration,
        ...
        # Update private request time (cooldown)
        await db.update_private_request_time(message.from_user.id)

        # Schedule auto‑delete
        if config.AUTO_DELETE_TIME > 0 and sent:
            to_delete = [message.id, smsg.id, sent.id]
            asyncio.create_task(
                auto_delete_messages(client, message.chat.id, to_delete, config.AUTO_DELETE_TIME)
            )
...
async def button_callbacks(client: Client, callback_query: CallbackQuery):
    ...
    if data == "dev_info":
        ...
        await client.edit_message_caption(
            chat_id=message.chat.id, message_id=message.id,
            caption=help_txt, reply_markup=InlineKeyboardMarkup(buttons),
            parse_mode=enums.ParseMode.HTML
        )
    elif data == "about_btn":
        buttons = [[InlineKeyboardButton("⬅️ Back to Home", callback_data="start_btn")]]
        await client.edit_message_caption(
            chat_id=message.chat.id, message_id=message.id,
            caption=script.ABOUT_TXT, reply_markup=InlineKeyboardMarkup(buttons),
            parse_mode=enums.ParseMode.HTML
        )
    elif data == "start_btn":
        bot = await client.get_me()
