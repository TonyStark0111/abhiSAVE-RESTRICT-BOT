...
# Helper for start image (config or fallback)
async def get_start_image():
    if config.START_IMG_URL:
        return config.START_IMG_URL
    return "https://i.postimg.cc/kX9tjGXP/16.png"   # default fallback
...
async def send_help(client: Client, message: Message):
    ...
    help_txt = script.HELP_TXT.format(
        config.PUBLIC_DAILY_LIMIT,
        config.PRIVATE_DAILY_LIMIT,
        config.PUBLIC_COOLDOWN,
        config.PRIVATE_COOLDOWN
    )
        ...
        if is_public_link:
            ...
            if is_limit_reached:
                ...
                return await message.reply_photo(
                    photo=SUBSCRIPTION,
                    caption=script.LIMIT_REACHED.format(config.PUBLIC_DAILY_LIMIT),
                    reply_markup=btn,
                    parse_mode=enums.ParseMode.HTML
                )

        # 2. Cooldown check (separate)
        if is_public_link:
            cooldown_remaining = await db.check_public_cooldown(message.from_user.id)
            if cooldown_remaining > 0:
                return await message.reply_text(
                    f"<b>⏳ Public request cooldown: please wait {cooldown_remaining} second(s).</b>\n\n"
                    "<i>This helps prevent spam and floodwaits.</i>",
                    parse_mode=enums.ParseMode.HTML
                )
        else:
            cooldown_remaining = await db.check_private_cooldown(message.from_user.id)
            if cooldown_remaining > 0:
                return await message.reply_text(
                    f"<b>⏳ Private request cooldown: please wait {cooldown_remaining} second(s).</b>\n\n"
                    "<i>This helps prevent spam and floodwaits.</i>",
                    parse_mode=enums.ParseMode.HTML
                )
        ...
        for msgid in range(fromID, toID + 1):
            ...
            if is_public_link:
                ...
                try:
                    ...
                    pass
            # If we reach here, it's a private/batch link requiring login
            # Check private daily limit before proceeding
            if await db.check_private_limit(message.from_user.id):
                btn = InlineKeyboardMarkup([[InlineKeyboardButton("💎 Upgrade to Premium", callback_data="buy_premium")]])
                await message.reply_photo(
                    photo=SUBSCRIPTION,
                    caption=script.PRIVATE_LIMIT_REACHED.format(config.PRIVATE_DAILY_LIMIT),
                    reply_markup=btn,
                    parse_mode=enums.ParseMode.HTML
                )
                batch_temp.IS_BATCH[message.from_user.id] = True
...
async def handle_restricted_content(client: Client, acc, message: Message, chat_target, msgid):
    ...
    if msg_type == "Text":
        try:
            ...
            return
    # Increment private traffic counter
    ...
    try:
        ...
        # Append auto‑delete notice if enabled
        ...
        # Send the file and capture the sent message
        ...
        # Update private request time (cooldown) after successful send
